...
function indekseGoreFenomen(liste, i) {
  fenomen = liste[i].profile;
  console.log(`${i}. indekste bulunan fenomen: ${fenomen}`);
}
...
function profilListesi(chosenList) {
  const newList = chosenList.slice();
  for (let i = 0; i < chosenList.length; i++) {
    newList[i] = newList[i].profile;
  }
  return newList;
}
...
function fenomenSil(chosenList, i) {
  const newList = chosenList.slice();
...
function fenomenEkle(chosenList, number, profile, followers, posts, platform) {
  const newList = chosenList.slice();
  const newItem =  {
    "number": 6,
    "profile": "Workintech",
...
fenomenEkle(fenomenler, number, profile, followers, posts, platform);
